...
model = dict(
    ...
    backbone=dict(
        ...
        out_indices=(0, 1, 2, 3),
        frozen_stages=1,
        norm_cfg=dict(type='BN', requires_grad=True), # added
        norm_eval=True, # added
        style='pytorch'),
    neck=dict(
        type='FPN',
...
#         max_norm=35,
#         norm_type=2))
optimizer_config = dict(grad_clip=None) # new
lr_config = dict(
    policy='step',
    warmup='linear',
    warmup_iters=500,
    warmup_ratio=0.001, #1. / 3,
    step=[8, 11])
...
# runtime settings
# total_epochs = 12
runner = dict(type='EpochBasedRunner', max_epochs=12) # new
